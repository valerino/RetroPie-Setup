...
function depends_lr-mess-exl100() {
...
function sources_lr-mess-exl100() {
	true
}

function build_lr-mess-exl100() {
	true
}

function install_lr-mess-exl100() {
	true
}

function configure_lr-mess-exl100() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="exl100"
	...
	addEmulator 1 "$md_id" "$_system" "$_script $_retroarch_bin $_mess $_config exl100 $biosdir -cart %ROM%"
