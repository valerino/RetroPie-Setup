...
rp_module_id="lr-mess-fmtmarty"
rp_module_name="Fujitsu FM Towns Marty"
rp_module_ext=".cue .chd .toc .nrg .gdi .iso .cdr"
...
function depends_lr-mess-fmtmarty() {
...
function sources_lr-mess-fmtmarty() {
	true
}

function build_lr-mess-fmtmarty() {
	true
}

function install_lr-mess-fmtmarty() {
	true
}

function configure_lr-mess-fmtmarty() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="fmtmarty"
	...
	addEmulator 1 "$md_id" "$_system" "$_script $_retroarch_bin $_mess $_config fmtmarty $biosdir -cdrm %ROM%"
