...
rp_module_id="lr-mess-samcoupe"
rp_module_name="M.G.T. Sam Coupé"
...
samcoupe.zip\n\n
How to load games:\n
type 'BOOT'<ret> to load disk\n"

rp_module_licence="GPL2 https://raw.githubusercontent.com/libretro/mame/master/LICENSE.md"
rp_module_section="exp"
rp_module_flags=""

function depends_lr-mess-samcoupe() {
...
function sources_lr-mess-samcoupe() {
	true
}

function build_lr-mess-samcoupe() {
	true
}

function install_lr-mess-samcoupe()  {
	true
}

function configure_lr-mess-samcoupe() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="samcoupe"
...
#  -autoboot_command \n"BOOT\n" -autoboot_delay 2	
	addEmulator 1 "$md_id" "$_system" "$_script $_retroarch_bin $_mess $_config samcoupe $biosdir -flop1 %ROM%"
