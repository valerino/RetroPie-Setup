...
rp_module_id="lr-mess-sv8000"
rp_module_name="Super Vision 8000"
...
function depends_lr-mess-sv8000() {
...
function sources_lr-mess-sv8000() {
	true
}

function build_lr-mess-sv8000() {
	true
}

function install_lr-mess-sv8000() {
	true
}

function configure_lr-mess-sv8000() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="sv8000"
	...
	addEmulator 1 "$md_id" "$_system" "$_script $_retroarch_bin $_mess $_config sv8000 $biosdir -cart %ROM%"
