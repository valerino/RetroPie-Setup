...
rp_module_id="lr-mess-gamepock"
rp_module_name="EPOCH GamePocket Computer"
...
function depends_lr-mess-gamepock() {
...
function sources_lr-mess-gamepock() {
	true
}

function build_lr-mess-gamepock() {
	true
}

function install_lr-mess-gamepock() {
	true
}

function configure_lr-mess-gamepock() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="gamepock"
