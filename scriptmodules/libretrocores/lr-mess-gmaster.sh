...
rp_module_id="lr-mess-gmaster"
rp_module_name="Hartung Game Master"
...
function depends_lr-mess-gmaster() {
...
function sources_lr-mess-gmaster() {
	true
}

function build_lr-mess-gmaster() {
	true
}

function install_lr-mess-gmaster() {
	true
}

function configure_lr-mess-gmaster() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="gmaster"
	...
	addEmulator 1 "$md_id" "$_system" "$_script $_retroarch_bin $_mess $_config gmaster $biosdir -cart %ROM%"
