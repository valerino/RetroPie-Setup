...
rp_module_id="lr-mess-studio2"
rp_module_name="RCA Studio II"
rp_module_ext=".zip .bin .rom .st2"
...
function depends_lr-mess-studio2() {
...
function sources_lr-mess-studio2() {
	true
}

function build_lr-mess-studio2() {
	true
}

function install_lr-mess-studio2() {
	true
}

function configure_lr-mess-studio2() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="studio2"
	...
	addEmulator 0 "$md_id" "$_system" "$_script $_retroarch_bin $_mess $_config studio2 $biosdir -cart %ROM%"
	addEmulator 1 "$md_id-no_media" "$_system" "$_script $_retroarch_bin $_mess $_config studio2 $biosdir"
