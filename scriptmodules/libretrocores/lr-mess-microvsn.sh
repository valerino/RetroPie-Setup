...
rp_module_id="lr-mess-microvsn"
rp_module_name="Milton Bradley Microvision"
...
function depends_lr-mess-microvsn() {
...
function sources_lr-mess-microvsn() {
	true
}

function build_lr-mess-microvsn() {
	true
}

function install_lr-mess-microvsn() {
	true
}

function configure_lr-mess-microvsn() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="microvsn"
	...
	addEmulator 1 "$md_id" "$_system" "$_script $_retroarch_bin $_mess $_config microvsn $biosdir -cart %ROM%"
