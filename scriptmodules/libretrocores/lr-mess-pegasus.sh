...
rp_module_id="lr-mess-pegasus"
rp_module_name="Amber Pegasus"
...
function depends_lr-mess-pegasus() {
...
function sources_lr-mess-pegasus() {
	true
}

function build_lr-mess-pegasus() {
	true
}

function install_lr-mess-pegasus() {
	true
}

function configure_lr-mess-pegasus() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="pegasus"
	...
	addEmulator 0 "$md_id-cart" "$_system" "$_script $_retroarch_bin $_mess $_config $_system $biosdir -cart1 %ROM%"
