...
rp_module_id="lr-mess-standalone"
rp_module_name="Classic Standalone Games"
rp_module_ext=".spec"
...
function depends_lr-mess-standalone() {
...
function sources_lr-mess-standalone() {
	true
}

function build_lr-mess-standalone() {
	true
}

function install_lr-mess-standalone() {
	true
}

function configure_lr-mess-standalone() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="standalone"
	...
	local _script="$scriptdir/scriptmodules/run_mess_standalone.sh"
	...
	addEmulator 1 "$md_id" "$_system" "$_script $_retroarch_bin $_mess $_config standalone $biosdir %ROM%"
