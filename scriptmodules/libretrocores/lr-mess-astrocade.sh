...
rp_module_id="lr-mess-astrocade"
rp_module_name="Bally Professional Arcade (Astrocade)"
...
function depends_lr-mess-astrocade() {
...
function sources_lr-mess-astrocade() {
	true
}

function build_lr-mess-astrocade() {
	true
}

function install_lr-mess-astrocade() {
	true
}

function configure_lr-mess-astrocade() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="astrocade"
	...
	addEmulator 1 "$md_id" "$_system" "$_script $_retroarch_bin $_mess $_config astrocde $biosdir -cart %ROM%"
