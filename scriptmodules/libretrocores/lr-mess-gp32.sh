...
rp_module_id="lr-mess-gp32"
rp_module_name="Game Park GP32"
rp_module_ext=".zip .smc "
...
function sources_lr-mess-gp32() {
	true
}

function build_lr-mess-gp32() {
	true
}

function install_lr-mess-gp32() {
	true
}

function configure_lr-mess-gp32() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	...
	local _script="$scriptdir/scriptmodules/run_mess.sh"
        ...
        # default to the most compatible BIOS, can be overridden within the MESS settings for the rom
        local _bios="-bios 166m"
	...
	# add the emulators.cfg as normal, pointing to the above script
        addEmulator 0 "$md_id" "$_system" "$_script $_retroarch_bin $_mess $_config $_system $biosdir $_bios -memc %ROM%"
