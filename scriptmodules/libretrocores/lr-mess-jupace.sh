...
rp_module_id="lr-mess-jupace"
rp_module_name="Jupiter Cantab Ace"
rp_module_ext=".zip .tap .wav"
...
function depends_lr-mess-jupace() {
...
function sources_lr-mess-jupace() {
	true
}

function build_lr-mess-jupace() {
	true
}

function install_lr-mess-jupace() {
	true
}

function configure_lr-mess-jupace() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="jupace"
	...
	addEmulator 1 "$md_id" "$_system" "$_script $_retroarch_bin $_mess $_config jupace $biosdir -cass %ROM%"
