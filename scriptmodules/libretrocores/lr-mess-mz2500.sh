...
rp_module_id="lr-mess-mz2500"
rp_module_name="Sharp MZ-2500"
rp_module_ext=".zip .fdi .td0 .imd .cqm .dsk .d77 .d88 .1dd"
...
function depends_lr-mess-mz2500() {
...
function sources_lr-mess-mz2500() {
	true
}

function build_lr-mess-mz2500() {
	true
}

function install_lr-mess-mz2500() {
	true
}

function configure_lr-mess-mz2500() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="mz2500"
	...
	addEmulator 1 "$md_id" "$_system" "$_script $_retroarch_bin $_mess $_config $_system $biosdir -flop1 %ROM%"
