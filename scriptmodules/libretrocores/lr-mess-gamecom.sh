...
rp_module_id="lr-mess-gamecom"
rp_module_name="Tiger Game.com"
rp_module_ext=".zip .bin .tgc"
...
function depends_lr-mess-gamecom() {
...
function sources_lr-mess-gamecom() {
	true
}

function build_lr-mess-gamecom() {
	true
}

function install_lr-mess-gamecom() {
	true
}

function configure_lr-mess-gamecom() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="gamecom"
	...
	addEmulator 1 "$md_id" "$_system" "$_script $_retroarch_bin $_mess $_config gamecom $biosdir -cart1 %ROM%"
