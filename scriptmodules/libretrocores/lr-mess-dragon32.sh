...
rp_module_id="lr-mess-dragon32"
rp_module_name="Dragon 32"
rp_module_ext=".zip .dsk .cas .rom"
...
dragon32.zip\n\n
How to load games:\n
http://www.progettoemma.net/mess/system.php?machine=dragon32"

rp_module_licence="GPL2 https://raw.githubusercontent.com/libretro/mame/master/LICENSE.md"
rp_module_section="exp"
rp_module_flags=""

function depends_lr-mess-dragon32() {
...
function sources_lr-mess-dragon32() {
	true
}

function build_lr-mess-dragon32() {
	true
}

function install_lr-mess-dragon32()  {
	true
}

function configure_lr-mess-dragon32() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="dragon32"
	...
	addEmulator 1 "$md_id-cass" "$_system" "$_script $_retroarch_bin $_mess $_config dragon32 $biosdir -cass %ROM%"
	addEmulator 0 "$md_id-disk" "$_system" "$_script $_retroarch_bin $_mess $_config dragon32 $biosdir -flop1 %ROM%"
	addEmulator 0 "$md_id-cart" "$_system" "$_script $_retroarch_bin $_mess $_config dragon32 $biosdir -cart %ROM%"
