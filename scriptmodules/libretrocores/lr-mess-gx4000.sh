...
rp_module_id="lr-mess-gx4000"
rp_module_name="Amstrad GX4000"
...
function depends_lr-mess-gx4000() {
...
function sources_lr-mess-gx4000() {
	true
}

function build_lr-mess-gx4000() {
	true
}

function install_lr-mess-gx4000()  {
	true
}

function configure_lr-mess-gx4000() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="gx4000"
	...
	addEmulator 1 "$md_id" "$_system" "$_script $_retroarch_bin $_mess $_config gx4000 $biosdir -cart %ROM%"
