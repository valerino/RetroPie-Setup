...
rp_module_id="lr-mess-sc3000"
rp_module_name="Sega SC-3000"
...
function depends_lr-mess-sc3000() {
...
function sources_lr-mess-sc3000() {
	true
}

function build_lr-mess-sc3000() {
	true
}

function install_lr-mess-sc3000() {
	true
}

function configure_lr-mess-sc3000() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="sc-3000"
	...
	addEmulator 1 "$md_id" "$_system" "$_script $_retroarch_bin $_mess $_config sc3000 $biosdir -cart %ROM%"
