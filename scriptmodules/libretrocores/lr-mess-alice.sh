...
rp_module_id="lr-mess-alice"
rp_module_name="Matra & Hackette Alice"
...
function depends_lr-mess-alice() {
...
function sources_lr-mess-alice() {
	true
}

function build_lr-mess-alice() {
	true
}

function install_lr-mess-alice() {
	true
}

function configure_lr-mess-alice() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	...
	addEmulator 1 "$md_id" "$_system" "$_script $_retroarch_bin $_mess $_config alice $biosdir -cass %ROM%"
