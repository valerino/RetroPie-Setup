...
rp_module_id="lr-mess-tutor"
rp_module_name="Tomy Tutor"
...
function depends_lr-mess-tutor() {
...
function sources_lr-mess-tutor() {
	true
}

function build_lr-mess-tutor() {
	true
}

function install_lr-mess-tutor() {
	true
}

function configure_lr-mess-tutor() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	...
	addEmulator 1 "$md_id" "$_system" "$_script $_retroarch_bin $_mess $_config tutor $biosdir -cart %ROM%"
