...
rp_module_id="lr-mess-electron"
rp_module_name="Acorn Electron"
rp_module_ext=".zip .wav .csw .uef .bin"
...
function depends_lr-mess-electron() {
...
function sources_lr-mess-electron() {
	true
}

function build_lr-mess-electron() {
	true
}

function install_lr-mess-electron()  {
	true
}

function configure_lr-mess-electron() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="electron"
	...
	addEmulator 1 "$md_id-cass" "$_system" "$_script $_retroarch_bin $_mess $_config electron $biosdir -cass %ROM%"
	#addEmulator 0 "$md_id-cart" "$_system" "$_script $_retroarch_bin $_mess $_config electron $biosdir -cart %ROM%"
