...
rp_module_id="lr-mess-to7"
rp_module_name="Thomson TO7"
rp_module_ext=".zip .wav .k7 .m7 .rom .fd .sap .qd .fdi .td0 .imd .cqm .dsk .d77 .d88 .1dd"
...
to7.zip, to7d.zip\n\n
Press B on menu screen to autoboot disks."

rp_module_licence="GPL2 https://raw.githubusercontent.com/libretro/mame/master/LICENSE.md"
rp_module_section="exp"
rp_module_flags=""

function depends_lr-mess-to7() {
...
function sources_lr-mess-to7() {
	true
}

function build_lr-mess-to7() {
	true
}

function install_lr-mess-to7() {
	true
}

function configure_lr-mess-to7() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	...
	# add the emulators.cfg as normal, pointing to the above script
        addEmulator 0 "$md_id-cass" "$_system" "$_script $_retroarch_bin $_mess $_config $_system $biosdir -cass %ROM%"
        addEmulator 0 "$md_id-cart" "$_system" "$_script $_retroarch_bin $_mess $_config $_system $biosdir -cart %ROM%"
        addEmulator 1 "$md_id-flop" "$_system" "$_script $_retroarch_bin $_mess $_config $_system $biosdir -flop1 %ROM%"
