...
rp_module_id="lr-mess-atom"
rp_module_name="Acorn Archimedes"
rp_module_ext=".zip .dsk .uef .wav .tap"
...
function sources_lr-mess-atom() {
	true
}

function build_lr-mess-atom() {
	true
}

function install_lr-mess-atom() {
	true
}

function configure_lr-mess-atom() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	...
	addEmulator 1 "$md_id-disk" "$_system" "$_script $_retroarch_bin $_mess $_config atom $biosdir -flop1 %ROM%"
	addEmulator 0 "$md_id-cass" "$_system" "$_script $_retroarch_bin $_mess $_config atom $biosdir -cass %ROM%"
