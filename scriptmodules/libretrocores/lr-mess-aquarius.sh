...
rp_module_id="lr-mess-aquarius"
rp_module_name="Mattel Aquarius"
rp_module_ext=".zip .bin .wav"
...
function depends_lr-mess-aquarius() {
...
function sources_lr-mess-aquarius() {
	true
}

function build_lr-mess-aquarius() {
	true
}

function install_lr-mess-aquarius() {
	true
}

function configure_lr-mess-aquarius() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="aquarius"
	...
	addEmulator 1 "$md_id-cart" "$_system" "$_script $_retroarch_bin $_mess $_config aquarius $biosdir -cart %ROM%"
        addEmulator 0 "$md_id-cass" "$_system" "$_script $_retroarch_bin $_mess $_config aquarius $biosdir -cass %ROM%"
