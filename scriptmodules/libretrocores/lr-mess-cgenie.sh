...
rp_module_id="lr-mess-cgenie"
rp_module_name="EACA Colour Genie EG-2000"
...
function depends_lr-mess-cgenie() {
...
function sources_lr-mess-cgenie() {
	true
}

function build_lr-mess-cgenie() {
	true
}

function install_lr-mess-cgenie() {
	true
}

function configure_lr-mess-cgenie() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="cgenie"
	...
	addEmulator 1 "$md_id" "$_system" "$_script $_retroarch_bin $_mess $_config cgenie $biosdir -cass %ROM%"
