...
rp_module_id="lr-mess-archimedes"
rp_module_name="Acorn Archimedes"
rp_module_ext=".zip .adf"
...
function depends_lr-mess-archimedes() {
...
function sources_lr-mess-archimedes() {
	true
}

function build_lr-mess-archimedes() {
	true
}

function install_lr-mess-archimedes()  {
	true
}

function configure_lr-mess-archimedes() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="archimedes"
	...
	addEmulator 1 "$md_id-A305" "$_system" "$_script $_retroarch_bin $_mess $_config aa305 $biosdir -flop %ROM%"
	addEmulator 1 "$md_id-A310" "$_system" "$_script $_retroarch_bin $_mess $_config aa310 $biosdir -flop %ROM%"
