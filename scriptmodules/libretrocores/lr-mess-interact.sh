...
rp_module_id="lr-mess-interact"
rp_module_name="Interact Family Computer"
rp_module_ext=".zip .wav .k7 .cin .for"
...
function depends_lr-mess-interact() {
...
function sources_lr-mess-interact() {
	true
}

function build_lr-mess-interact() {
	true
}

function install_lr-mess-interact() {
	true
}

function configure_lr-mess-interact() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="interact"
	...
	addEmulator 1 "$md_id" "$_system" "$_script $_retroarch_bin $_mess $_config interact $biosdir -cass %ROM%"
