...
rp_module_id="lr-mess-supracan"
rp_module_name="Funtech Super A'Can"
...
function depends_lr-mess-supracan() {
...
function sources_lr-mess-supracan() {
	true
}

function build_lr-mess-supracan() {
	true
}

function install_lr-mess-supracan() {
	true
}

function configure_lr-mess-supracan() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="supracan"
	...
	addEmulator 1 "$md_id" "$_system" "$_script $_retroarch_bin $_mess $_config supracan $biosdir -cart %ROM%"
