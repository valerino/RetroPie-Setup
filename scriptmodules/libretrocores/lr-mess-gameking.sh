...
rp_module_id="lr-mess-gameking"
rp_module_name="Timetop GameKing"
...
function depends_lr-mess-gameking() {
...
function sources_lr-mess-gameking() {
	true
}

function build_lr-mess-gameking() {
	true
}

function install_lr-mess-gameking() {
	true
}

function configure_lr-mess-gameking() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="gameking"
	...
	addEmulator 1 "$md_id" "$_system" "$_script $_retroarch_bin $_mess $_config gameking $biosdir -cart %ROM%"
