...
rp_module_id="lr-mess-sf7000"
rp_module_name="Sega Super Control Station SF-7000"
rp_module_ext=".zip .sf7"
...
function depends_lr-mess-sf7000() {
...
function sources_lr-mess-sf7000() {
	true
}

function build_lr-mess-sf7000() {
	true
}

function install_lr-mess-sf7000() {
	true
}

function configure_lr-mess-sf7000() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="sf-7000"
	...
	# add the emulators.cfg as normal, pointing to the above script
        addEmulator 0 "$md_id" "$_system" "$_script $_retroarch_bin $_mess $_config sf7000 $biosdir -flop %ROM%"
