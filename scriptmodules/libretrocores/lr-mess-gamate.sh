...
rp_module_id="lr-mess-gamate"
rp_module_name="Bit Corporation Gamate"
...
function depends_lr-mess-gamate() {
...
function sources_lr-mess-gamate() {
	true
}

function build_lr-mess-gamate() {
	true
}

function install_lr-mess-gamate() {
	true
}

function configure_lr-mess-gamate() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="gamate"
	...
	addEmulator 1 "$md_id" "$_system" "$_script $_retroarch_bin $_mess $_config gamate $biosdir -cart %ROM%"
