...
rp_module_id="lr-mess-multivision"
rp_module_name="Othello Multivision 1000"
...
function depends_lr-mess-multivision() {
...
function sources_lr-mess-multivision() {
	true
}

function build_lr-mess-multivision() {
	true
}

function install_lr-mess-multivision() {
	true
}

function configure_lr-mess-multivision() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="multivision"
	...
	addEmulator 1 "$md_id" "$_system" "$_script $_retroarch_bin $_mess $_config omv1000 $biosdir -cart %ROM%"
