...
rp_module_id="lr-mess-arcadia"
rp_ext=".zip .bin"
rp_sysname="Emerson Arcadia 2001"
rp_module_desc="MESS emulator ($rp_sysname) - MESS Port for libretro"
rp_module_help="ROM Extensions: $rp_ext\n\n
Put games in:\n
$romdir/arcadia\n\n
...
function depends_lr-mess-arcadia() {
...
function sources_lr-mess-arcadia() {
	true
}

function build_lr-mess-arcadia() {
	true
}

function install_lr-mess-arcadia() {
	true
}

function configure_lr-mess-arcadia() {
	_mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	...
	addSystem "$_system" "$rp_sysname" "$rp_ext"
}
