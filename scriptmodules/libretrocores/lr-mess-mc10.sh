...
rp_module_id="lr-mess-mc10"
rp_module_name="Tandy TRS-80 MC-10"
...
function sources_lr-mess-mc10() {
	true
}

function build_lr-mess-mc10() {
	true
}

function install_lr-mess-mc10() {
	true
}

function configure_lr-mess-mc10() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	...
	addEmulator 1 "$md_id-mc10" "$_system" "$_script $_retroarch_bin $_mess $_config mc10 $biosdir -cass %ROM%"
