...
rp_module_id="lr-mess-lynx48k"
rp_module_name="Camputers Lynx"
rp_module_ext=".zip .tap"
...
lynx48k.zip, lynx96k.zip, lynx128k.zip"

rp_module_licence="GPL2 https://raw.githubusercontent.com/libretro/mame/master/LICENSE.md"
rp_module_section="exp"
rp_module_flags=""

function depends_lr-mess-lynx48k() {
...
function sources_lr-mess-lynx48k() {
	true
}

function build_lr-mess-lynx48k() {
	true
}

function install_lr-mess-lynx48k() {
	true
}

function configure_lr-mess-lynx48k() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	local _retroarch_bin="$rootdir/emulators/retroarch/bin/retroarch"
	local _system="lynx48k"
	...
	addEmulator 1 "$md_id-48k" "$_system" "$_script $_retroarch_bin $_mess $_config lynx48k $biosdir -cass %ROM%"
	addEmulator 0 "$md_id-96k" "$_system" "$_script $_retroarch_bin $_mess $_config lynx96k $biosdir -cass %ROM%"
	addEmulator 0 "$md_id-128k" "$_system" "$_script $_retroarch_bin $_mess $_config lynx128k $biosdir -cass %ROM%"
