...
rp_module_id="lr-mess-m5"
rp_module_name="Sord M5"
rp_module_ext=".zip .wav .cas .bin .rom"
...
function sources_lr-mess-m5() {
	true
}

function build_lr-mess-m5() {
	true
}

function install_lr-mess-m5()  {
	true
}

function configure_lr-mess-m5() {
	local _mess=$(dirname "$md_inst")/lr-mess/mess_libretro.so
	...
	addEmulator 1 "$md_id-cart" "$_system" "$_script $_retroarch_bin $_mess $_config m5p $biosdir -cart1 %ROM%"
	addEmulator 0 "$md_id-cass" "$_system" "$_script $_retroarch_bin $_mess $_config m5p $biosdir -cass %ROM%"
